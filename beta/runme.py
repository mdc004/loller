...
def handleClient(conn, addr):
    ...
    conn.send(b"\nSono disponibili 4 campioni, ognuno di essi si differenza per danno, punti salute e guarigione.")
    conn.send(b"\nIl tuo obbiettivo e' di fare piu' danno possibile al tuo avversario, cossicche' arrivi a 0 (zero) punti salute.")
    conn.send(b"\nOgni turno puoi usare una sola abilita', non puoi usare la stessa abilita' per due turni di seguito.")
    conn.send(b"\nSai le regole! Sei pronto a partire!\n")
    time.sleep(1)
    conn.send(b"\nEcco i campioni")
    for champ in champions:
        conn.send(f"\nNome: {champ[0]} | punti salute: {champ[1]} | autoattacco: {champ[2]} | guarigione: {champ[3]} | danno avanzato: {champ[4]}".encode())
    ...
    while playersStack[whoAmI][1] > 0 and playersStack[enemyID][1] > 0:
        if playersStack[whoAmI][3] == True:
            ...
            if playersStack[whoAmI][2] != 0 or playersStack[enemyID][2] != 0:
                conn.send(f"\nIl tuo avversario ha usato abilita' {ABILITY[playersStack[enemyID][2]]}".encode())
            ...
            while ability == playersStack[whoAmI][2] or ability > 4 or ability < 2:
                conn.send(b"\nNon puoi eseguire la stessa mossa per due turni di fila")
                conn.send(b"\nPuoi inserire solo 1, 2 o 3")
                conn.send(b"\nLa tua mossa (1 autoattacco - 2 guarigione - 3 danno avanzato): ")
                ability = int(conn.recv(64)) + 1

            playersStack[whoAmI][2] = ability
            # Swapping turn
